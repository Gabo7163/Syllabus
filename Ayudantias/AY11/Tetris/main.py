...
if __name__ == '__main__':
    ...
    cliente = Cliente(p.RUTA_CANCION)
    ...
    ventana_inicio.senal_verificar_usuario.connect(cliente.verificar_usuario)
    cliente.senal_mensaje_error.connect(ventana_error.mostrar)
    cliente.senal_empezar_juego.connect(ventana_juego.mostrar_ventana)
    cliente.senal_empezar_juego.connect(logica_juego.comenzar_partida)
    cliente.senal_empezar_juego.connect(ventana_inicio.salir)

    # Auto conexión
    cliente.senal_empezar_juego.connect(cliente.iniciar_musica)
    ...
    # Enviar puntaje al servidor cuando el juego termina
    logica_juego.senal_servidor_puntaje.connect(cliente.enviar_puntaje)
