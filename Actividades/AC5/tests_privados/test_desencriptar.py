...
class TestDesencriptar(unittest.TestCase):
    ...
    def test_decodificar_largo_pequeño(self):
        ...
        """
        test = decodificar_largo(bytearray(b"\x00\x00\x03"))
        res = 3
        # Verificar tipo de dato pedido
    ...
    def test_decodificar_largo_grande(self):
        ...
        """
        test = decodificar_largo(bytearray(b"\xA1\x11\xAA"))
        res = 10555818
        # Verificar tipo de dato pedido
        self.assertIsInstance(test, int)
        # Verificar resultados
    ...
    def test_separar_msg_encriptado_par(self):
        ...
        with patch("main.decodificar_largo", side_effect=new_decodificar_largo):
            test = separar_msg_encriptado(
                bytearray(
                    b"\x00\x00\x02\x02\x02\x03\x00\x01\x00\x00\x00\x01\x00\x00\x04"
                )
            )
            res = [
                bytearray(b"\x02\x02"),
    ...
    def test_separar_msg_encriptado_impar(self):
        ...
        with patch("main.decodificar_largo", side_effect=decodificar_largo):
            test = separar_msg_encriptado(
                bytearray(
                    b"\x00\x00\x03\x44\x02\x02\x03\x00\x01\x02\x00\x00\x00\x01\x00\x00\x05"
                )
            )
            res = [
                bytearray(b"\x02\x02\x44"),
    ...
    def test_decodificar_rango_pequeño(self):
        ...
        """
        test = decodificar_rango(bytearray(b"\x00\x00\x02\x00\x00\x03"))
        res = [2, 3]
        # Verificar tipo de dato pedido
        self.assertIsInstance(res, list)
        # Verificar resultados
    ...
    def test_decodificar_rango_grande(self):
        ...
        """
        test = decodificar_rango(bytearray(b"\x00\x00\x0C\x00\x11\x5C"))
        res = [12, 4444]
        # Verificar tipo de dato pedido
        self.assertIsInstance(res, list)
        # Verificar resultados
        ...
        with patch("main.decodificar_largo", side_effect=decodificar_largo):
            with patch(
                ...
                with patch("main.decodificar_rango", side_effect=decodificar_rango):
                    test = desencriptar(
                        bytearray(
                            b"\x00\x00\x03\x05\x02\x80\x09\x00\x01\x02\x08\x00\x00\x01\x00\x00\x03"
                        )
                    )
                    res = bytearray(b"\x09\x80\x02\x05\x08")
                    # Verificar tipo de dato pedido
