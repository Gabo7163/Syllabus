...
class TestFiltrarPeliculasGenero(unittest.TestCase):
    ...
    def generador_generos(self, variacion):
        ...
        if variacion == 1:
            ...
            yield Genero('Accion', 4)
            yield Genero('Drama', 4)
            yield Genero('Crimen', 4)
            yield Genero('Drama', 5)
            yield Genero('Accion', 6)
            yield Genero('Drama', 6)
            yield Genero('Accion', 7)
            yield Genero('Sci-Fi', 7)
            yield Genero('Biografia', 8)
            yield Genero('Crimen', 8)
            yield Genero('Drama', 8)
            yield Genero('Crimen', 13)
            yield Genero('Drama', 13)
            yield Genero('Aventura', 16)
            yield Genero('Drama', 16)
            yield Genero('Sci-Fi', 16)
            yield Genero('Comedy', 21)
            yield Genero('Crimen', 21)
    ...
    def test_filtrar_por_genero_1(self):
        ...
        resultado_esperado = [
            (Pelicula(5, 'El club de la pelea', 'David Fincher', 1999, 8.8), Genero('Drama', 5)),
            (Pelicula(20, 'Red Social', 'David Fincher', 2010, 7.7), Genero('Drama', 20))
        ]
    ...
    def test_filtrar_por_genero_none(self):
        ...
        resultado_esperado = [
            (Pelicula(4, 'Batman: el caballero de la noche', 'Christopher Nolan', 2008, 9.0), Genero('Accion', 4)),
            (Pelicula(5, 'El club de la pelea', 'David Fincher', 1999, 8.8), Genero('Drama', 5)),
            (Pelicula(7, 'Matrix', 'Lana Wachowski', 1999, 8.7), Genero('Sci-Fi', 7)),
            (Pelicula(8, 'Buenos muchachos', 'Martin Scorsese', 1990, 8.7), Genero('Biografia', 8))
        ]
        self.assertSequenceEqual(list(datos), resultado_esperado)
