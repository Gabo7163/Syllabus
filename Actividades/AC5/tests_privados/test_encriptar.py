...
class TestEncriptar(unittest.TestCase):
    ...
    def test_verificar_rango_None(self):
        ...
        """
        mensaje = bytearray(b"\x00\x01\x09\x03")
    ...
    def test_verificar_rango_minimo(self):
        ...
        """
        mensaje = bytearray(b"\x00\x01\x09\x03")
        self.assertRaises(AttributeError, verificar_rango, mensaje, -11, 2)

    def test_verificar_rango_maximo(self):
        """
        Verificar excepción cuando fin es mayor al largo
        """
        mensaje = bytearray(b"\x00\x01\xCA\xFE")
        self.assertRaises(AttributeError, verificar_rango, mensaje, 1, 4444)
    ...
    def test_verificar_rango_creciente(self):
        ...
        self.assertRaises(AttributeError, verificar_rango, mensaje, 2, 0)

    def test_codificar_secuencia_pequeño(self):
        """
        Verificar correcta codificación con números pequeños
        """
        test = codificar_rango(1, 11)
        res = bytearray(b"\x00\x00\x01\x00\x00\x0B")
        # Verificar tipo de dato pedido
        self.assertIsInstance(test, bytearray)
        # Verificar resultados
        self.assertEqual(test, res)
    ...
    def test_codificar_secuencia_grande(self):
        ...
        """
        test = codificar_rango(2, 4444)
        res = bytearray(b"\x00\x00\x02\x00\x11\\")
        # Verificar tipo de dato pedido
        self.assertIsInstance(test, bytearray)
        # Verificar resultados
        self.assertEqual(test, res)
    ...
    def test_codificar_largo_pequeño(self):
        ...
        """
        test = codificar_largo(11)
        res = bytearray(b"\x00\x00\x0B")
        # Verificar tipo de dato pedido
        self.assertIsInstance(test, bytearray)
        # Verificar resultados
        self.assertEqual(test, res)
    ...
    def test_codificar_largo_grande(self):
        ...
        """
        test = codificar_largo(4243)
        res = bytearray(b"\x00\x10\x93")
        # Verificar tipo de dato pedido
        self.assertIsInstance(test, bytearray)
        # Verificar resultados
        self.assertEqual(test, res)
    ...
    def test_separar_msg(self):
        ...
        """
        test = separar_msg(bytearray(b"\x00\x01\x02\x09"), 1, 3)
        res = [bytearray(b"\x09\x02\x01"), bytearray(b"\x00\x00\x01\x02")]
        # Verificar tipo de dato pedido
    ...
    def test_separar_msg_2(self):
        ...
        """
        test = separar_msg(bytearray(b"\x00\x01\x02\x09"), 1, 2)
        res = [bytearray(b"\x01\x02"), bytearray(b"\x00\x00\x01\x09")]
        # Verificar tipo de dato pedido
        self.assertIsInstance(res, list)
        # Verificar resultados
        self.assertListEqual(test, res)
    ...
    def test_separar_msg_3(self):
        ...
        """
        test = separar_msg(bytearray(b"\x08\x01\x0A\x0B"), 0, 2)
        res = [bytearray(b"\x0A\x01\x08"), bytearray(b"\x00\x01\x02\x0B")]
        # Verificar tipo de dato pedido
        self.assertIsInstance(res, list)
        # Verificar resultados
        self.assertListEqual(test, res)
